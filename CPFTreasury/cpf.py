...
class CPS_TREASURY_INTERFACE(InterfaceScore):
    ...
    def deposit_proposal_fund(self, _ipfs_key: str, _total_installment_count: int, _title: str, _sponsor_address: str,
                           _contributor_address: str, _total_budget: int):
        pass
...
class CPF(IconScoreBase):
    ...
    @external
    def transfer_proposal_fund_to_cps_treasury(self, _ipfs_key: str, _total_installment_count: int, _title: str, _sponsor_address: str,
                              _contributor_address: str, _total_budget: int = 0) -> None:
        ...
        if _ipfs_key not in self._proposals_keys:
            ...
            try:
                ...
                cps_treasury_score.icx(total_transfer).deposit_proposal_fund(_ipfs_key, _total_installment_count, _title,
                                                                                       _sponsor_address,
    ...
    def update_proposal_fund(self, _ipfs_key: str, _added_budget: int = 0, _total_installment_count: int = 0) -> None:
        ...
        if _ipfs_key in self._proposals_keys:
            ...
            self._proposals_details[_ipfs_key][self._TOTAL_INSTALLMENT_COUNT] = str(int(self._proposals_details[_ipfs_key][
                                                                                    self._TOTAL_INSTALLMENT_COUNT]) + _total_installment_count)

            try:
                cps_treasury_score = self.create_interface_score(self._cps_treasury_score.get(), CPS_TREASURY_INTERFACE)
                cps_treasury_score.icx(total_transfer).update_proposal_fund(_ipfs_key, _added_budget, _total_installment_count)
    ...
    def disqualify_proposal_fund(self, _ipfs_key: str) -> None:
        ...
        if self._proposals_details[_ipfs_key][self._STATUS] != self._ACTIVE:
            revert('The project isnot in active state')

        if _ipfs_key in self._proposals_keys:
            self._proposals_details[_ipfs_key][self._STATUS] = self._DISQUALIFIED
            self._proposals_details[_ipfs_key][self._TOTAL_BUDGET] = str(int(self._proposals_details[_ipfs_key][
                                                                                    self._TOTAL_BUDGET]) - int(self.msg.value))
